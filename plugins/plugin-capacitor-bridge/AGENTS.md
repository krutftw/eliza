...
- **Interactive-over-background text lane (#11914).** Both TEXT handler paths (bionic UDS delegation and the renderer device bridge) route every generate through the process-wide `InferencePriorityGate` (`@elizaos/core`): interactive turns (the default) wait FIFO by owner; requests marked `priority: "background"` run only when the lane is idle and otherwise fail immediately with `InferenceLaneBusyError`. Background work is clamped to the RAM-class budget (`resolveMobileLaneBudget`, driven by the `ELIZA_INFERENCE_RAM_CLASS` env contract from #11760) before decode. It never enters a timed queue, so autonomous jobs cannot stack behind the bionic host's resident lock.
